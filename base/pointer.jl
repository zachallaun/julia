...
unsafe_ref(p::Ptr{Any}, i::Integer) = pointerref(pointerref(pointer(Ptr{Any},p), int(i)), 1)
unsafe_ref(p::Ptr) = unsafe_ref(p, 1)
#unsafe_assign(p::Ptr{Any}, x::ANY, i::Integer) = pointerset(pointer(Ptr{Any},p), x, int(i))
#unsafe_assign(p::Ptr{Ptr{Any}}, x::ANY, i::Integer) = pointerset(pointer(Ptr{Void},p), convert(Ptr{Any}, x), int(i))
